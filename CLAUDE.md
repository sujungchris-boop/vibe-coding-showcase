...
## 작업 스타일 (세션 공통 — 어느 환경에서 작업하든 유지)
- **주도적 PO 모드**: 소유자(나 국장)가 방향만 주면 구현→검증→커밋→푸시→배포 확인까지 알아서 완주. 되돌릴 수 있는 일은 묻지 말고 진행, 파괴적/대외적 결정만 확인.
- **디자인 언어**: 사이트는 아기자기 파스텔(Jua/Gaegu 폰트, 핑크 계열) 유지. 학생 작품 자체는 고유 미감 존중.
- **검증 원칙**: 요소 존재가 아니라 **실제 동작**(클릭→상태 변화)으로 검증. 게임류는 정적 HUD 텍스트·장식 캔버스에 속지 말 것. 인라인 `<script>` 안에 literal `</script>` 금지(모듈 통째로 끊김 — 실사고 있었음).
- **숨김 탭 rAF**: 미리보기·헤드리스 탭에선 requestAnimationFrame이 발화하지 않음 — 남의 코드 검증 시 전역 함수를 수동 호출(`for(...) updateGame()`)로 프레임을 돌려 상태 변화를 단언.
- **flame-of-gospel.html**(레포 루트): 대표 시연작 "복음의 불길"(Chris 명의로 Firestore `works`에 게시됨). 플랫폼 코드가 아니라 작품 콘텐츠.
